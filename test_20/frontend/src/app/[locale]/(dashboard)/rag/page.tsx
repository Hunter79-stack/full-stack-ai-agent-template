...
export default function RAGPage() {
  ...
  const availableTypes = [
    ...new Set(searchResults.map((r) => r.metadata?.filetype).filter(Boolean)),
  ];
    ...
    <div className="container mx-auto max-w-6xl">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Manage RAG collections and search documents
          </p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-1">
          ...
          {isLoading ? (
            <Card className="text-muted-foreground p-6 text-center text-sm">Loading...</Card>
            ...
            <Card className="p-6 text-center">
              ...
              <p className="text-muted-foreground mt-1 text-xs">Use CLI to ingest documents</p>
            </Card>
          ) : (
            <div className="space-y-2">
              {collections.length > 1 && (
                <Card
                  className={`hover:bg-accent cursor-pointer p-3 transition-colors ${selectedCollection === "__all__" ? "border-primary bg-accent" : ""}`}
                  onClick={() => {
                ...
                >
                  ...
                  <p className="text-muted-foreground text-xs">
                    Search across {collections.length} collections
                  </p>
                </Card>
              )}
              {collections.map((col) => (
                <Card
                  key={col.name}
                  className={`hover:bg-accent cursor-pointer p-3 transition-colors ${selectedCollection === col.name ? "border-primary bg-accent" : ""}`}
                  onClick={() => {
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="min-w-0">
                      ...
                      {col.info && (
                        <p className="text-muted-foreground text-xs">
                          {col.info.total_vectors.toLocaleString()} vectors
                        </p>
                      )}
                    </div>
                    <Button
          ...
          <Card className="p-4">
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={!selectedCollection || isSearching || !searchQuery.trim()}
              >
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              >
                ...
                {availableTypes
                  .filter((t) => !["pdf", "docx", "md", "txt"].includes(t as string))
                  .map((t) => (

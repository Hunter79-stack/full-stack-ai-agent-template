...
```bash
# Local files
uv run test_20 rag-ingest /path/to/docs/ --collection documents --recursive
# Google Drive
uv run test_20 rag-sync-gdrive --collection documents --folder-id <id>
```

### Search

```bash
uv run test_20 rag-search "your query" --collection documents
```

### Manage collections

```bash
uv run test_20 rag-collections   # List all
uv run test_20 rag-stats          # Show stats
uv run test_20 rag-drop <name>    # Delete collection
```
...
*Generated with [Full-Stack AI Agent Template](https://github.com/vstorm-co/full-stack-ai-agent-template) v0.2.2*

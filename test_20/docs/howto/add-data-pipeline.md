...
Data pipelines are background jobs that fetch, transform, and store data on a schedule. They use the existing background task system (celery).
...
```bash
# Manual run via CLI
uv run test_20 cmd run-pipeline sync-products
